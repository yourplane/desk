...
- `desk list` (validates `desk` is installed; the build symlinks it to `/usr/local/bin` for SSM’s non-login shell and copies `config.ini` to `/root/.config/desk/` because test commands run as root).
