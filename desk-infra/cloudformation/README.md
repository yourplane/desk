...
- **502 Bad Gateway** on **`{name}-{port}.your-apex`**:
  ...
  - **After `Host` is correct, 502 usually means Caddy matched the route but the reverse-proxy upstream failed** (connection refused or bad response). On the **managed router** EC2, each public route uses **`127.0.0.1:<local_port>`** fed by **SSM port forwarding** from the workstation. **Check:** the **SSM session** for that route is still running (`desk route` / process for the forward), the **workstation dev server** is listening on the **remote** port (e.g. Vite **`5173`**), and **`desk web-router probe`** on the router instance succeeds for that hostname. If **`desk-router`** was deployed with an empty **`WebRouterBaseDomain`**, redeploy **`desk-router`** with **`DESK_CUSTOM_DOMAIN_NAME`** set so **`DESK_WEB_ROUTER_BASE_DOMAIN`** matches **`your-apex`** (Caddy hostnames must align with the browser **`Host`**).
