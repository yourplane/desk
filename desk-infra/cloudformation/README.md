...
- **502 Bad Gateway** on **`{name}-{port}.your-apex`** after auth works: the web-router behavior must use an origin request policy that **forwards the viewer `Host`** (e.g. **Managed-AllViewer**). **Managed-AllViewerExceptHostHeader** replaces **`Host`** with the internal ALB name, so Caddy cannot match **`Host: {name}-{port}.desk.example.com`** and routing fails (often seen as **502** from the load balancer / CloudFront).
