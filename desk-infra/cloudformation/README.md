...
2. This stack adds a **second CloudFront distribution** whose alias is **`*.CustomDomainName`** (e.g. `*.desk.example.com`). It uses a **VPC origin** to that internal ALB. A **CloudFront Function** on viewer-request checks **`request.cookies`** for **`desk_token`** or **`desk_web_gate`** (the raw **`Cookie`** header is **not** exposed on **`request.headers`** in CloudFront Functions); if neither is present, it redirects to **`/session-bridge.html?return=…`** on the apex so tokens can refresh and the user returns to the same port URL.
...
- **New tab jumps back to the desk apex:** The web-router viewer function redirects to **`/session-bridge.html?return=…`** when neither **`desk_token`** nor **`desk_web_gate`** is present. Normally the **session keeper** refreshes cookies in the background (desk SPA timer and/or **`session-keeper.js`** injected into proxied HTML). Port links open a **new tab**, which does not share **sessionStorage**. The SPA sets a **host-only** **`desk_token`** on the desk apex (large JWT, not duplicated onto **`Domain=`**) and a small **`desk_web_gate`** (**SHA-256** hex) with **`Domain=.desk.example.com`** and **`SameSite=None; Secure`**, so requests to **`{name}-{port}.desk.example.com`** send a **short** cookie header (large combined headers can be dropped or mishandled at the edge). Reload after deploy; clear site cookies if needed.
- **502 Bad Gateway** on **`{name}-{port}.your-apex`**:
  - **Wrong `Host` at Caddy (fixed in stack):** the web-router cache behavior must use an origin request policy that **forwards the viewer `Host`** (**Managed-AllViewer**, ID `216adef6-5c7f-47e4-b989-5492eafa07d3`). **Managed-AllViewerExceptHostHeader** sends the internal ALB DNS as **`Host`**, so Caddy’s **`header_regexp Host`** does not match — you would see **404** (“No matching desk route”) or mis-routing, not a working proxy.
  - **After `Host` is correct, 502 often meant the internal ALB had no healthy targets:** the web router **must listen on `0.0.0.0:8780`** (or all interfaces), not loopback-only. A systemd unit with **`DESK_WEB_ROUTER_LISTEN=:8780`** (port-only) makes Caddy **`bind 127.0.0.1`** — the ALB then cannot connect on the instance’s **private IP**, health checks fail, and CloudFront sees **502**. Use **`DESK_WEB_ROUTER_LISTEN=0.0.0.0:8780`** (and a non-localhost **`DESK_WEB_ROUTER_BASE_DOMAIN`**) on the managed router, or reinstall **`desk web-router start --on-boot`** with those env vars. New **`desk-cli`** builds treat **`:port`** + public base domain as **listen on all interfaces** without requiring `0.0.0.0` in the unit file.
  - **Upstream still fails (502 after ALB is healthy):** On the **managed router** EC2, each public route uses **`127.0.0.1:<local_port>`** fed by **SSM port forwarding** from the workstation. **Check:** the **SSM session** for that route is still running, the **workstation dev server** is listening on the **remote** port (e.g. Vite **`5173`**), and **`desk web-router probe`** on the router instance succeeds for that hostname. If **`desk-router`** was deployed with an empty **`WebRouterBaseDomain`**, redeploy **`desk-router`** with **`DESK_CUSTOM_DOMAIN_NAME`** set so **`DESK_WEB_ROUTER_BASE_DOMAIN`** matches **`your-apex`** (Caddy hostnames must align with the browser **`Host`**).
  - **Vite** (and similar) may block unknown hosts: set **`server.allowedHosts: true`** (or list your `*.desk.example.com` host) in **`vite.config`**, or the app may not respond correctly through the proxy.
