...
2. This stack adds a **second CloudFront distribution** whose alias is **`*.CustomDomainName`** (e.g. `*.desk.example.com`). It uses a **VPC origin** to that internal ALB. A **CloudFront Function** on viewer-request requires the **`desk_token`** or **`desk_web_gate`** cookie; otherwise it redirects to the SPA origin.
...
- **New tab jumps back to the desk apex:** The web-router viewer function redirects to **`https://your-apex/`** when neither **`desk_token`** nor **`desk_web_gate`** is present on the request. Port links open a **new tab**, which does not share **sessionStorage**; only cookies (with **`Domain=.desk.example.com`**) are sent. **`desk_web_gate`** is a short **SHA-256** cookie derived from the ID token so gating still works when **`desk_token`** is too large for the browser’s per-cookie limit (~4KB). Reload the desk app once after deploy so cookies are re-written (**`SameSite=None; Secure`** when a shared domain is configured). Clear stale **host-only** cookies for the apex in devtools if redirects persist.
