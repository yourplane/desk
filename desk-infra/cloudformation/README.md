...
- **New tab jumps back to the desk apex:** The web-router viewer function redirects to **`https://your-apex/`** when neither **`desk_token`** nor **`desk_web_gate`** is present in the **Cookie** header seen at CloudFront. Port links open a **new tab**, which does not share **sessionStorage**. The SPA sets a **host-only** **`desk_token`** on the desk apex (large JWT, not duplicated onto **`Domain=`**) and a small **`desk_web_gate`** (**SHA-256** hex) with **`Domain=.desk.example.com`** and **`SameSite=None; Secure`**, so requests to **`{name}-{port}.desk.example.com`** send a **short** cookie header (large combined headers can be dropped or mishandled at the edge). Reload after deploy; clear site cookies if needed.
