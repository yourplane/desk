...
- **PKCE verifier**: Stored in sessionStorage and a short-lived cookie when redirecting to login so it survives the redirect back (e.g. same or new tab). On HTTPS, those cookies use `Secure`; `HttpOnly` is not set from the SPA (`document.cookie` cannot mark HttpOnly—only a server `Set-Cookie` can).
