...
function routerInfraBannerMessage(status: RouterInfraStatus): string | null {
  if (status.phase === 'waking') {
    return `Router infra is ${status.friendly_label.toLowerCase()}. Public web routes may take several minutes to become reachable — retry your link until then.`
  }
  if (status.phase === 'idle' && status.demand) {
    return 'Router infra is stopped but needed for open web routes. Wake is in progress or will run on the next reaper tick.'
  }
  if (status.phase === 'sleeping') {
    return `Router infra is ${status.friendly_label.toLowerCase()}.`
  }
